...
export function TeacherCard({ teacher }: TeacherCardProps) {
  return (
    <Link
      ...
      className="block no-underline bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-primary/10"
    >
      ...
      {teacher.subject && (
        <p className="text-center text-sm text-primary">{teacher.subject}</p>
      )}
    </Link>
  );
}
